...
(function () {
  ...
  async function fetchBatch(params, markerIds, runToken) {
    ...
    const response = await fetch('<?=$assetBase?>/api/dns-checker.php?' + batchParams.toString(), {
      headers: {
  ...
  async function runCheck() {
    ...
    try {
      ...
      const nextUrl = '<?=$assetBase?>/pages/whois_dns_checker.php?domain=' + encodeURIComponent(query);
