  ...
  <div class="mx-auto max-w-7xl">
    ...
    <div class="mt-6 rounded-3xl border border-outline-variant/20 bg-white/90 p-4 shadow-sm md:p-5">
      <div class="flex flex-col gap-3 md:flex-row md:items-center">
        ...
        <a class="rounded-full border border-outline-variant/30 px-6 py-3 text-xs font-bold uppercase tracking-[0.16em] text-primary hover:border-black" href="<?=$assetBase?>/pages/whois_premium_domain_intelligence_landing_page.php">Back to landing</a>
      </div>
...
(function () {
    ...
    items.forEach((item) => {
      ...
      card.innerHTML =
        ...
        '<div class="mt-4 flex flex-wrap items-center gap-2">' +
          '<a class="inline-flex items-center justify-center rounded-full border border-outline-variant/30 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-primary hover:border-black" href="<?=$assetBase?>/pages/whois_ai_domain_search.php?query=' + encodeURIComponent(String(item.domain || '')) + '">Check availability</a>' +
          purchaseButton +
        '</div>';
  ...
  async function generate() {
    ...
    try {
      const response = await fetch('<?=$assetBase?>/api/name-generator.php', {
        method: 'POST',
        headers: {
